...
function isRelevantObject(object) {
  ...
  return (
    !config.excluded.includes(geometryType) &&
    !config.excluded.includes(objectType)
  );
}
...
function isSphereObject(object) {
  return (
    object.geometry.type === "IcosahedronGeometry" ||
...
export function findSpheresInCube(cube) {
  ...
  // Handle both wireframe and solid cube cases
  const targetCube = cube.wireframe || cube;
  
  // Ensure the cube's matrix is up to date
  if (targetCube.updateMatrixWorld) {
      targetCube.updateMatrixWorld(true);
  }
  
  const cubeBox = new THREE.Box3().setFromObject(targetCube);
