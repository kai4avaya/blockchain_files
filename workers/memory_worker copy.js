...
async function openDB(dbName, version = undefined) {
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = function (event) {
      dbInstance = event.target.result;
      console.log(`Database ${dbName} opened successfully with version ${dbInstance.version}`);
      resolve(dbInstance);
    };
...
self.onmessage = async function(event) {
  ...
  try {
    ...
    // self.postMessage({ id, error: error.message + ' | with ' + event.data  });
    self.postMessage({ id, error: error.message + ' | with ' + JSON.stringify(event.data) });
  }
};