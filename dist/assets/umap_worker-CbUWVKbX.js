...
utils$5.reshape2d = utils$5.rejectionSample = utils$5.max2d = utils$5.max = utils$5.mean = utils$5.sum = utils$5.linear = utils$5.ones = utils$5.zeros = utils$5.filled = utils$5.range = utils$5.empty = utils$5.norm = utils$5.tauRand = utils$5.tauRandInt = void 0;
function tauRandInt(n, random) {
    return Math.floor(random() * n);
}
utils$5.tauRandInt = tauRandInt;
function tauRand(random) {
    return random();
}
utils$5.tauRand = tauRand;
function norm(vec) {
    var e_1, _a;
    var result = 0;
    try {
        for (var vec_1 = __values$3(vec), vec_1_1 = vec_1.next(); !vec_1_1.done; vec_1_1 = vec_1.next()) {
...
utils$5.max2d = max2d;
function rejectionSample$1(nSamples, poolSize, random) {
...
utils$5.rejectionSample = rejectionSample$1;
...
Object.defineProperty(heap$2, "__esModule", { value: true });
heap$2.smallestFlagged = heap$2.deheapSort = heap$2.buildCandidates = heap$2.uncheckedHeapPush = heap$2.heapPush = heap$2.rejectionSample = heap$2.makeHeap = void 0;
var utils$4 = __importStar$4(utils$5);
function makeHeap(nPoints, size) {
    var makeArrays = function (fillValue) {
        return utils$4.empty(nPoints).map(function () {
            return utils$4.filled(size, fillValue);
        });
    };
    var heap = [];
...
heap$2.makeHeap = makeHeap;
function rejectionSample(nSamples, poolSize, random) {
    var result = utils$4.zeros(nSamples);
    for (var i = 0; i < nSamples; i++) {
        var rejectSample = true;
        var j = 0;
        while (rejectSample) {
            j = utils$4.tauRandInt(poolSize, random);
            var broken = false;
            for (var k = 0; k < i; k++) {
                if (j === result[k]) {
                    broken = true;
...
heap$2.rejectionSample = rejectionSample;
...
heap$2.uncheckedHeapPush = uncheckedHeapPush;
...
heap$2.buildCandidates = buildCandidates;
...
heap$2.smallestFlagged = smallestFlagged;
...
var _a;
Object.defineProperty(matrix$3, "__esModule", { value: true });
matrix$3.getCSR = matrix$3.normalize = matrix$3.eliminateZeros = matrix$3.multiplyScalar = matrix$3.maximum = matrix$3.subtract = matrix$3.add = matrix$3.pairwiseMultiply = matrix$3.identity = matrix$3.transpose = matrix$3.SparseMatrix = void 0;
var utils$3 = __importStar$3(utils$5);
var SparseMatrix = (function () {
    ...
    SparseMatrix.prototype.getRows = function () {
        return Array.from(this.entries, function (_a) {
            var _b = __read$2(_a, 2); _b[0]; var value = _b[1];
            return value.row;
        });
    };
    ...
    SparseMatrix.prototype.toArray = function () {
        var _this = this;
        var rows = utils$3.empty(this.nRows);
        var output = rows.map(function () {
            return utils$3.zeros(_this.nCols);
        });
        this.entries.forEach(function (value) {
...
matrix$3.SparseMatrix = SparseMatrix;
...
matrix$3.transpose = transpose;
function identity(size) {
    var _a = __read$2(size, 1), rows = _a[0];
...
matrix$3.subtract = subtract;
function maximum(a, b) {
    return elementWise(a, b, function (x, y) { return (x > y ? x : y); });
}
matrix$3.maximum = maximum;
function multiplyScalar(a, scalar) {
    return a.map(function (value) {
        return value * scalar;
    });
}
matrix$3.multiplyScalar = multiplyScalar;
...
matrix$3.eliminateZeros = eliminateZeros;
function normalize(m, normType) {
    ...
    try {
        for (var _b = __values$2(colsByRow.keys()), _c = _b.next(); !_c.done; _c = _b.next()) {
            var row = _c.value;
            _loop_1(row);
        }
    }
...
Object.defineProperty(tree$2, "__esModule", { value: true });
tree$2.searchFlatTree = tree$2.makeLeafArray = tree$2.makeForest = tree$2.FlatTree = void 0;
var utils$2 = __importStar$2(utils$5);
...
function euclideanRandomProjectionSplit(data, indices, random) {
    ...
    var leftIndex = utils$2.tauRandInt(indices.length, random);
    var rightIndex = utils$2.tauRandInt(indices.length, random);
    rightIndex += leftIndex === rightIndex ? 1 : 0;
    rightIndex = rightIndex % indices.length;
    var left = indices[leftIndex];
    var right = indices[rightIndex];
    var hyperplaneOffset = 0;
    var hyperplaneVector = utils$2.zeros(dim);
    ...
    var nLeft = 0;
    var nRight = 0;
    var side = utils$2.zeros(indices.length);
    for (var i = 0; i < indices.length; i++) {
        ...
        if (margin === 0) {
            side[i] = utils$2.tauRandInt(2, random);
            if (side[i] === 0) {
                nLeft += 1;
            }
            else {
    ...
    var indicesLeft = utils$2.zeros(nLeft);
    var indicesRight = utils$2.zeros(nRight);
    nLeft = 0;
    nRight = 0;
...
function flattenTree(tree, leafSize) {
    ...
    var hyperplanes = utils$2
        .range(nNodes)
        .map(function () { return utils$2.zeros(tree.hyperplane ? tree.hyperplane.length : 0); });
    var offsets = utils$2.zeros(nNodes);
    var children = utils$2.range(nNodes).map(function () { return [-1, -1]; });
    var indices = utils$2
        .range(nLeaves)
        .map(function () { return utils$2.range(leafSize).map(function () { return -1; }); });
...
function recursiveFlatten(tree, hyperplanes, offsets, children, indices, nodeNum, leafNum) {
    ...
    if (tree.isLeaf) {
        ...
        (_a = indices[leafNum]).splice.apply(_a, __spread$1([0, tree.indices.length], tree.indices));
        leafNum += 1;
...
function makeLeafArray(rpForest) {
    ...
    if (rpForest.length > 0) {
        var output = [];
        try {
            for (var rpForest_1 = __values$1(rpForest), rpForest_1_1 = rpForest_1.next(); !rpForest_1_1.done; rpForest_1_1 = rpForest_1.next()) {
                var tree = rpForest_1_1.value;
                output.push.apply(output, __spread$1(tree.indices));
            }
        }
        catch (e_1_1) { e_1 = { error: e_1_1 }; }
        finally {
...
tree$2.makeLeafArray = makeLeafArray;
function selectSide(hyperplane, offset, point, random) {
    ...
    if (margin === 0) {
        var side = utils$2.tauRandInt(2, random);
        return side;
    }
    else if (margin > 0) {
        return 0;
...
tree$2.searchFlatTree = searchFlatTree;
...
Object.defineProperty(nn_descent, "__esModule", { value: true });
nn_descent.initializeSearch = nn_descent.makeInitializedNNSearch = nn_descent.makeInitializations = nn_descent.makeNNDescent = void 0;
var heap$1 = __importStar$1(heap$2);
var matrix$2 = __importStar$1(matrix$3);
var tree$1 = __importStar$1(tree$2);
var utils$1 = __importStar$1(utils$5);
...
nn_descent.makeNNDescent = makeNNDescent;
...
nn_descent.makeInitializations = makeInitializations;
function makeInitializedNNSearch(distanceFn) {
    return function nnSearchFn(data, graph, initialization, queryPoints) {
        var e_1, _a;
        var _b = matrix$2.getCSR(graph), indices = _b.indices, indptr = _b.indptr;
        for (var i = 0; i < queryPoints.length; i++) {
...
nn_descent.makeInitializedNNSearch = makeInitializedNNSearch;
...
nn_descent.initializeSearch = initializeSearch;
...
var Matrix_1 = matrix$1.Matrix = Matrix$1;
...
var _default = matrix$1.default = Matrix$1;
matrix$1.determinant = determinant;
var inverse_1 = matrix$1.inverse = inverse$1;
...
Object.defineProperty(umap, "__esModule", { value: true });
umap.initTransform = umap.resetLocalConnectivity = umap.fastIntersection = umap.findABParams = umap.cosine = umap.euclidean = umap.UMAP = void 0;
var heap = __importStar(heap$2);
var matrix = __importStar(matrix$3);
var nnDescent = __importStar(nn_descent);
var tree = __importStar(tree$2);
...
umap.findABParams = findABParams;
...
umap.fastIntersection = fastIntersection;
...
umap.resetLocalConnectivity = resetLocalConnectivity;
...
umap.initTransform = initTransform;
...
function normalizeAndScaleCoordinates(coordinates, sceneBoundingBox, zOffset = 0) {
